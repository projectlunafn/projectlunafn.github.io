...
!(function ($) {
    ...
    $.download_XMLHttpRequest = function (url, fn, data, method) { // èŽ·å¾—urlå’Œdata
        var xhr = new XMLHttpRequest();
        xhr.open(method, url, true);//getè¯·æ±‚ï¼Œè¯·æ±‚åœ°å€ï¼Œæ˜¯å¦å¼‚æ­¥
        xhr.responseType = "blob";    // è¿”å›žç±»åž‹blob
        xhr.onload = function () {// è¯·æ±‚å®Œæˆå¤„ç†å‡½æ•°
            if (this.status === 200) {
                var blob = this.response;// èŽ·å–è¿”å›žå€¼
    ...
    function pasteLink(isReturn) {
        ...
        '<span><i class="icon icon-clear"></i>Clear</span>' === a.innerHTML ? (c.value = "", a.innerHTML = '<span><i class="icon icon-paste"></i>Paste</span>') :
            ...
            navigator.clipboard.readText().then(function (f) {
                    ...
                    console.log("/næ ¼å¼" + isTikLink);
                    console.log("/næ ¼å¼" + isDouyinLink);

                    if (!isTikLink && !isDouyinLink && !isTwitterLink && !isMxtakatakLink) {
                        console.log("/né“¾æŽ¥æ ¼å¼é”™è¯¯");
                        isTikLink = false;
                        isDouyinLink = false;
                        //æ ¼å¼é”™è¯¯
   ...
   function getUID(fn,link){
    ...
    $("#bookmark").click(function () {
    	...
    	var add_bookmark_tips_1 = {
    		...
    		"ar": "Ø§Ø¶ØºØ· Ø¹Ù„Ù‰ Ctrl + D Ù„ÙˆØ¶Ø¹ Ø¥Ø´Ø§Ø±Ø© Ù…Ø±Ø¬Ø¹ÙŠØ© Ø¹Ù„Ù‰ Ù‡Ø°Ø§ Ø§Ù„Ù…ÙˆÙ‚Ø¹.",
    		"de": "DrÃ¼cken Sie Strg+D, um diese Seite mit einem Lesezeichen zu versehen.",
    		"es": "Presione Ctrl+D para marcar este sitio.",
    		"fa": "Ctrl+D Ø±Ø§ ÙØ´Ø§Ø± Ø¯Ù‡ÛŒØ¯ ØªØ§ Ø§ÛŒÙ† Ø³Ø§ÛŒØª Ø±Ø§ Ù†Ø´Ø§Ù†Ù‡ Ú¯Ø°Ø§Ø±ÛŒ Ú©Ù†ÛŒØ¯.",
    		...
    		"ja": "ã“ã®ã‚µã‚¤ãƒˆã‚’ãƒ–ãƒƒã‚¯ãƒžãƒ¼ã‚¯ã™ã‚‹ã«ã¯ã€Ctrl+D ã‚’æŠ¼ã—ã¦ãã ã•ã„ã€‚",
    		"ko": "ì´ ì‚¬ì´íŠ¸ë¥¼ ë¶ë§ˆí¬í•˜ë ¤ë©´ Ctrl+Dë¥¼ ëˆ„ë¥´ì‹­ì‹œì˜¤.",
    		"pt": "Pressione Ctrl+D para marcar este site como favorito.",
    		"ru": "ÐÐ°Ð¶Ð¼Ð¸Ñ‚Ðµ Ctrl+D, Ñ‡Ñ‚Ð¾Ð±Ñ‹ Ð´Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ ÑÑ‚Ð¾Ñ‚ ÑÐ°Ð¹Ñ‚ Ð² Ð·Ð°ÐºÐ»Ð°Ð´ÐºÐ¸.",
    		"th": "à¸à¸” Ctrl+D à¹€à¸žà¸·à¹ˆà¸­à¸šà¸¸à¹Šà¸à¸¡à¸²à¸£à¹Œà¸à¹„à¸‹à¸•à¹Œà¸™à¸µà¹‰",
    		"tr": "Bu siteye yer iÅŸareti koymak iÃ§in Ctrl+D tuÅŸlarÄ±na basÄ±n.",
    		"vi": "Nháº¥n Ctrl + D Ä‘á»ƒ Ä‘Ã¡nh dáº¥u trang web nÃ y.",
    		"zh-CN": "æŒ‰ Ctrl+D ç»„åˆé”®å°†æœ¬ç½‘ç«™åŠ å…¥ä¹¦ç­¾ã€‚",
    		"zh-TW": "æŒ‰ Ctrl+D çµ„åˆéµå°‡æœ¬ç¶²ç«™åŠ å…¥æ›¸ç±¤ã€‚"
    	};
    	var add_bookmark_tips_2 = {
        	"en": "Press Cmd+D to bookmark this site.",
        	"ar": "Ø§Ø¶ØºØ· Ø¹Ù„Ù‰ Cmd + D Ù„ÙˆØ¶Ø¹ Ø¥Ø´Ø§Ø±Ø© Ù…Ø±Ø¬Ø¹ÙŠØ© Ø¹Ù„Ù‰ Ù‡Ø°Ø§ Ø§Ù„Ù…ÙˆÙ‚Ø¹.",
    		"de": "DrÃ¼cken Sie Cmd+D, um diese Seite mit einem Lesezeichen zu versehen.",
    		"es": "Presione Cmd+D para marcar este sitio.",
    		"fa": "Ø¨Ø±Ø§ÛŒ Ù†Ø´Ø§Ù†Ú© Ú©Ø±Ø¯Ù† Ø§ÛŒÙ† Ø³Ø§ÛŒØªØŒ Cmd+D Ø±Ø§ ÙØ´Ø§Ø± Ø¯Ù‡ÛŒØ¯.",
    		...
    		"ja": "ã“ã®ã‚µã‚¤ãƒˆã‚’ãƒ–ãƒƒã‚¯ãƒžãƒ¼ã‚¯ã™ã‚‹ã«ã¯ã€Cmd+D ã‚’æŠ¼ã—ã¦ãã ã•ã„ã€‚",
    		"ko": "ì´ ì‚¬ì´íŠ¸ë¥¼ ë¶ë§ˆí¬í•˜ë ¤ë©´ Cmd+Dë¥¼ ëˆ„ë¥´ì‹­ì‹œì˜¤.",
    		"pt": "Pressione Cmd+D para marcar este site como favorito.",
    		"ru": "ÐÐ°Ð¶Ð¼Ð¸Ñ‚Ðµ Cmd+D, Ñ‡Ñ‚Ð¾Ð±Ñ‹ Ð´Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ ÑÑ‚Ð¾Ñ‚ ÑÐ°Ð¹Ñ‚ Ð² Ð·Ð°ÐºÐ»Ð°Ð´ÐºÐ¸.",
    		"th": "à¸à¸” Cmd+D à¹€à¸žà¸·à¹ˆà¸­à¸šà¸¸à¹Šà¸à¸¡à¸²à¸£à¹Œà¸à¹„à¸‹à¸•à¹Œà¸™à¸µà¹‰",
    		"tr": "Bu siteye yer iÅŸareti koymak iÃ§in Cmd+D tuÅŸlarÄ±na basÄ±n.",
    		"vi": "Nháº¥n Cmd+D Ä‘á»ƒ Ä‘Ã¡nh dáº¥u trang web nÃ y.",
        	"zh-CN": "æŒ‰ Cmd+D ç»„åˆé”®å°†æœ¬ç½‘ç«™åŠ å…¥ä¹¦ç­¾ã€‚",
        	"zh-TW": "æŒ‰ Cmd+D çµ„åˆéµå°‡æœ¬ç¶²ç«™åŠ å…¥æ›¸ç±¤ã€‚"
        };
